...
import org.viewer.util.context.VertxContext;
...
public class MutinyMinioClient {
  ...
  public Uni<GetObjectResponse> getObject(GetObjectArgs args) {
    return autoCreateBucket(args).flatMap(ignored -> createFrom().completionStage(supplier(() -> minioAsyncClient.getObject(args))));
  }
  ...
  public Uni<StatObjectResponse> statObject(StatObjectArgs statObjectArgs) throws InsufficientDataException, IOException, NoSuchAlgorithmException, InvalidKeyException, XmlParserException, InternalException {
    return autoCreateBucket(statObjectArgs).flatMap(ignored -> createFrom().completionStage(supplier(() -> minioAsyncClient.statObject(statObjectArgs))));
  }
  ...
  public Uni<ObjectWriteResponse> putObject(PutObjectArgs args) {
    return autoCreateBucket(args).flatMap(ignored -> createFrom().completionStage(supplier(() -> minioAsyncClient.putObject(args))));
  }

  public Uni<ObjectWriteResponse> uploadObject(UploadObjectArgs args) {
    return VertxContext.rollbackContext(() ->
        autoCreateBucket(args)
          .onItem()
          .transformToUni(ignored -> createFrom()
            .completionStage(supplier(() -> minioAsyncClient.uploadObject(args)))),
      vertx
    );
  }
  ...
  public Uni<String> getBucketPolicy(GetBucketPolicyArgs args) throws InsufficientDataException, InternalException, InvalidKeyException, IOException, NoSuchAlgorithmException, XmlParserException {
    return autoCreateBucket(args).flatMap(ignored -> createFrom().completionStage(supplier(() -> minioAsyncClient.getBucketPolicy(args))));
  }
  ...
  public Uni<ObjectWriteResponse> uploadSnowballObjects(UploadSnowballObjectsArgs args) throws InsufficientDataException, InternalException, InvalidKeyException, IOException, NoSuchAlgorithmException, XmlParserException {
    return autoCreateBucket(args).flatMap(ignore -> createFrom().completionStage(supplier(() -> minioAsyncClient.uploadSnowballObjects(args))));
  }

  public Uni<Void> autoCreateBucket(@NotNull BucketArgs bucketArgs) {
    return createFrom()
      .item(BucketExistsArgs.builder().bucket(bucketArgs.bucket()).build())
      .flatMap(args -> createFrom().completionStage(
        supplier(() -> minioAsyncClient.bucketExists(args))
      ))
      .log("Bucket exists for " + bucketArgs.bucket())
      .flatMap(isExists -> isExists
        ? createFrom().voidItem()
        : createFrom().completionStage(
        supplier(() -> minioAsyncClient.makeBucket(
          MakeBucketArgs.builder().bucket(bucketArgs.bucket()).build())))
      )
      .onFailure().invoke(e -> log.atError().log("Error checking or creating bucket: {}", bucketArgs.bucket(), e));
  }
}